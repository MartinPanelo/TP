...
function listarusers(regis) {
  ...
  let cantFilas =Math.max( regis.filter(item => (item.genero == "male")).length,
  ...
  regis.forEach((UnaPersona) => {
    ...
    if (UnaPersona.genero == "male") {
      document.getElementById("M+" + posM).innerHTML =
        ...
        </div>`; + document.getElementById("M+" + posM).innerHTML;
      posM += 1;
    } else {
...
function filtrarporañio(pais=paisCombo) {
  ...
  regitroFiltroanio = registro.filter((persona) => persona.anioRegistro > anio && 
  (persona.pais == pais || pais == undefined) );
